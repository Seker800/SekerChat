...
- **除非用户在当前任务中明确、直接要求“推送生产环境”、“部署生产环境”或同等含义的操作，任何 Agent 都严禁擅自将代码、镜像、配置或数据变更发布到生产环境。**
- “完成开发”、“修复完成”、“验证通过”、“创建本地提交”、“继续做完”或默认自动提交规则，**均不构成生产推送授权**。
- 生产推送授权不得从历史对话、旧任务、文档、自动化流程或 Agent 自身判断中推定；必须由用户针对当前任务直接下达。
- 如果指令是否包含生产推送存在任何歧义，必须视为**未授权**，立即停在本地提交或验证阶段并向用户确认，不得先推送后说明。
- 未获得上述明确授权时，不得执行任何生产部署脚本、上传生产镜像或构建产物、更新生产 compose/配置、对生产数据面执行写操作，或以其他方式使本地变更在生产生效。
